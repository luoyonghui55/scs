...
import com.hjlc.util.utils.TwoDimensionCode;
...
public class ToolController extends BaseController {
	...
	public ModelAndView editEmail() throws Exception{
		...
		pd = this.getPageData();
		mv.setViewName(PAGE_PATH_PREFIX + "system/tools/interfaceTest.jsp");
		mv.addObject("pd", pd);
		return mv;
	}
	...
	public ModelAndView goSendEmail() throws Exception{
		...
		PageData pd = new PageData();
		pd = this.getPageData();
		mv.setViewName(PAGE_PATH_PREFIX + "system/tools/email.jsp");
		mv.addObject("pd", pd);
		return mv;
	}
	...
	public ModelAndView goTwoDimensionCode() throws Exception{
		...
		PageData pd = new PageData();
		pd = this.getPageData();
		mv.setViewName(PAGE_PATH_PREFIX + "system/tools/twoDimensionCode.jsp");
		mv.addObject("pd", pd);
		return mv;
	}
	...
	public Object createTwoDimensionCode(){
		...
		PageData pd = new PageData();
		pd = this.getPageData();
		String errInfo = "success", encoderImgId = Tools.getUUID() +".png"; //encoderImgId此处二维码的图片名
		String encoderContent = pd.getString("encoderContent");				//内容
		if(null == encoderContent){
			errInfo = "error";
		}else{
			try {
				String filePath = PathUtil.getClasspath() + Constants.FILEPATHTWODIMENSIONCODE + encoderImgId;  //存放路径
	...
	public Object readTwoDimensionCode(){
		...
		if(null == imgId){
			...
			try {
				String filePath = PathUtil.getClasspath() + Constants.FILEPATHTWODIMENSIONCODE + imgId;  //存放路径
	...
	public ModelAndView ztree() throws Exception{
		...
		PageData pd = new PageData();
		pd = this.getPageData();
		mv.setViewName(PAGE_PATH_PREFIX + "system/tools/ztree.jsp");
		mv.addObject("pd", pd);
		return mv;
	}
	...
	public ModelAndView map() throws Exception{
		...
		PageData pd = new PageData();
		pd = this.getPageData();
		mv.setViewName(PAGE_PATH_PREFIX + "system/tools/map.jsp");
		mv.addObject("pd", pd);
		return mv;
	}
	...
	public ModelAndView mapXY() throws Exception{
		...
		PageData pd = new PageData();
		pd = this.getPageData();
		mv.setViewName(PAGE_PATH_PREFIX + "system/tools/mapXY.jsp");
		mv.addObject("pd", pd);
		return mv;
	}
